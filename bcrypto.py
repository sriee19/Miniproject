...
def get_crypto_data(symbol, start, end):
    try:
        ...
        ohlcv = exchange.fetch_ohlcv(symbol, timeframe='1d', since=int(start_timestamp), limit=500)
        ...
        st.error(f"An error occurred while fetching crypto data: {str(e)}")
        return None
...
n_days = n_years * 365
df = data[selected_stock_symbols + selected_crypto_symbols]
df['Prediction'] = df[selected_stock_symbols[0]].shift(-n_days)  # Assume the prediction is based on the first selected stock symbol

# Check if you have selected any cryptocurrencies
if selected_crypto_symbols:
    # Modify the prediction column for cryptocurrencies
    st.write("No cryptocurrencies selected. Prediction section is not available.") # Assume the prediction is based on the first selected crypto symbol

    # df['Prediction'] = df[selected_crypto_symbols[0]].shift(-n_days)  # Assume the prediction is based on the first selected crypto symbol
else:
    # Hide the prediction section if no cryptocurrencies are selected
    st.write("No cryptocurrencies selected. Prediction section is not available.") # Assume the prediction is based on the first selected crypto symbol
...
if len(X) > int(len(X) * 0.2):  # Check if data size is greater than 20% for testing
    ...
    predict_df = df[selected_stock_symbols + selected_crypto_symbols].tail(n_days)
...
# Note: Replace 'YOUR_NEWS_API_KEY' with your actual News API key
