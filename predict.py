...
if investment_type == 'Stock':
    investment_symbol = st.sidebar.selectbox('Select Stock Symbol:', available_stock_symbols)
else:
...
if selected_crypto_symbols:
    ...
    if not df.empty:
        df['Prediction'] = df[selected_stock_symbols[0]].shift(-n_months)  # Assume the prediction is based on the first selected symbol
        ...
        if len(X) > int(len(X) * 0.2):  # Check if data size is greater than 20% for testing
            ...
            predict_df = df[selected_stock_symbols].tail(n_months)
            ...
            prediction_df = df[selected_stock_symbols].tail(n_months).copy()
            prediction_df['Prediction'] = prediction

            st.subheader('Predicted Price for Next 12 Months')
            st.write(prediction_df)
            st.line_chart(prediction_df[selected_stock_symbols ]+ ['Prediction'])



            # Create a table for the prediction dates
            prediction_dates = pd.date_range(start=st.sidebar.date_input('Prediction Start Date', datetime.today()), periods=n_months, freq='M')
            prediction_dates_table = pd.DataFrame({'Month': prediction_dates, 'Prediction Date': prediction_dates.strftime('%B, %Y')})
            st.subheader('Prediction Dates for Next 12 Months')
            st.write(prediction_dates_table)
        else:
            st.write("Not enough data to perform the split. Please choose a larger date range.")

    # Create an overall prediction table
    if not selected_crypto_symbols and not df.empty:
        st.subheader('Overall Predicted Prices for Selected Stock Symbols')
        
        # Get the last prediction date from prediction_dates
        last_prediction_date = prediction_dates[-1].strftime('%B, %Y')
        
        overall_prediction_table = pd.DataFrame({
            'Stock Symbol': selected_stock_symbols,
            'Current Price': [data[symbol].iloc[-1] for symbol in selected_stock_symbols],
            'Predicted Date': [last_prediction_date for _ in selected_stock_symbols],  # Use the last prediction date
            'Predicted Price': [prediction_df[symbol].iloc[-1] for symbol in selected_stock_symbols]
        })
        st.write(overall_prediction_table)
